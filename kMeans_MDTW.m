function [member_id, centers, obj_opt, clusters] = kMeans_MDTW(A, K, best_init_id)
% distance: cDTW
...
%             centers(k,:) = meanavg(member_id, A, k); 
        ...
        for i = 1:m
            ...
            [Dist(i,:),~] = cDTW_kernel(A(i,:),centers,window);
        end

        [dist_to_center, member_id] = min(Dist,[],2);
        obj = sumsqr(dist_to_center);
        disp(obj);
        if norm(prev_mem - member_id) == 0 || abs(obj-prev_obj) < m*1e-5
            break;
        end
    end
